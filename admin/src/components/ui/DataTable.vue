...
<template>
  <div :class="{'vtable-reduced' : reduced}" class="vtable">
    ...
    <v-table theme="dark" class="vtable-desktop">
      <thead>
      ...
      <tr v-for="i in items" v-if="!loader">
        <td v-for="h in headers" :class="`text-${h.align || 'left'}`">
          <div v-if="!useSlots()[`item-${h.value}`]">{{ i[h.value] }}</div>
...
@keyframes shadowPulse {
  33% {
    background: #FFF;
    box-shadow: -24px 0 #ff00dd, 24px 0 #FFF;
  }
  66% {
    background: #ff00dd;
    box-shadow: -24px 0 #FFF, 24px 0 #FFF;
  ...
  100% {
    background: #FFF;
    box-shadow: -24px 0 #FFF, 24px 0 #ff00dd;
  }
}
