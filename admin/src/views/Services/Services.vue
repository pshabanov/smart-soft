...
const viewModel = ref<ServicesRequestType[]>([])
...
onMounted(async ()=>{
  ...
  const services = await ServicesService().services()
  isLoading.value = false
  viewModel.value = services.data
})

const updateModel = (itemId: number) => {
  router.push(`${ PAGES.SERVICES_DETAIL }/${ itemId }`)
}
...
const deleteModelConfirm = (itemId: number) => {
  ...
  isLoading.value = true
  ServicesService().servicesDelete(itemId).then(() => {
    viewModel.value = viewModel.value.filter(item=>item.id !== itemId)
    isLoading.value = false
  })
}
const createNews = () => {
  router.push(`${ PAGES.SERVICES_CREATE }`)
}
